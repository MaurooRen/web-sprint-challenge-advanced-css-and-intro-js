...
function getArtistByIndex(list, index) {
  let result
  result = `the artist at index ${list[index].id} is ${list[index].name}`
  return result
}  
...
function get20s(list){
  ...
  list.filter(function(item){
    if(parseInt(item.years.split(' - ')[0]) >= 1900 && parseInt(item.years.split(' - ')[1]) <= 2000){ newArray.push(item.name)}
  })
  return newArray
}
...
*/
function removeArtist(lista, index) {
